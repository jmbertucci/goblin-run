namespace Generator {
    ...
    export class JumpTables {
        ...
        private calculateJumpVelocities(): void {
            for (let step = 0; step <= Parameters.HEIGHT_STEPS; step++) {
                let height = this.getMaxHeightForStep(step);
         ...
         * @param step 
         */
        private getMaxHeightForStep(step: number) {
            return Parameters.HEIGHT_MIN + (Parameters.HEIGHT_MAX - Parameters.HEIGHT_MIN) / Parameters.HEIGHT_STEPS * step;
        }
         ...
         * @param height 
         */
        private getJumpVelocity(height: number) {
            return -Math.sqrt(2 * height * Parameters.GRAVITY);
        }
